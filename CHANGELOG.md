...
- Удалены неиспользуемые зависимости, устаревшие типы и шаблонные компоненты;
  удалены дублирующие изображения тайлов.
- `express` и `socket.io` перенесены в runtime-зависимости серверного workspace.
- Удалена неиспользуемая настройка SCSS-препроцессора.

## [1.14.0] - 2026-09-25

### Итоги аудита архитектуры, стабильности и инструкций агентам

1. **Согласованы сетка, данные тайлов и Socket.IO-контракты** - клиент
   использует серверный `gridSize` и переэкспортирует авторитетный каталог
   тайлов; события и ack-ответы приведены к единым типам и payload.
2. **Версионированы сохранения и усилена устойчивость сервера** - добавлены
   схема сохранений с миграцией legacy-данных, проверка восстановленного
   состояния и обработка ошибок сохранения/загрузки.
3. **Управляются подписки, таймеры и drag-обработчики** - обработчики
   Socket.IO отписываются при размонтировании, повторная регистрация устранена,
   отложенные throttle-вызовы и UI-ресурсы очищаются; добавлен тест отписки.
4. **Защищена и описана Socket.IO Admin UI** - доступ только с bcrypt
   credentials, по умолчанию только чтение; запуск сервера корректно
   обрабатывает ошибку восстановления и повторные сигналы завершения.
5. **Уменьшен начальный клиентский чанк** - панель правил и её данные
   загружаются отдельным чанком при первом открытии правил.
6. **Обновлены инструкции агентам** - `AGENTS.md` описывает архитектуру,
   доменные инварианты, сетевые контракты, игровые циклы, проверки и правила
   работы с сохранениями; дополнен инструкциями клиента и сервера.
...
- **Socket.IO Admin UI** - инструментирование включается только при заданных
  credentials; используется bcrypt-аутентификация и режим read-only по умолчанию.
- **`server/src/index.ts`** - ошибка загрузки сохранённых партий останавливает
  сервер с ненулевым кодом, повторный сигнал завершения не запускает shutdown снова.
- **Клиентские компоненты** - Socket.IO-подписки снимаются при размонтировании;
  drag-обработчики и отложенные таймеры очищаются при смене игры и размонтировании.
- **`src/modules/GameService.ts`** - `onGameUpdated` возвращает функцию отписки;
  `throttle` получил метод отмены отложенного вызова.
- **`src/App.vue` / `src/modules/GameService.ts`** - подписка на `gameUpdated`
  создаётся один раз и снимается при размонтировании; сокет-слушатели и
  отложенное позиционирование превью также очищаются.
- **`src/components/GameLobby/GameLobby.vue` / `src/components/Draggable.vue`** -
  таймер загрузки и обработчики drag завершаются до размонтирования.
- **`src/utils/common.ts`** - throttle получил `cancel()` для отмены отложенного
  запроса при завершении жизненного цикла компонента.
